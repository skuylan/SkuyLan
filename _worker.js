			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname) {
					...
					case `/sub/${userID_Path}`: {
						...
						const vlessSubConfig = getvlessSub(userID, request.headers.get('Host'));
...
function getvlessConfig(userIDs, hostName) {
	const commonUrlPart = `:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=/vless-ws#${hostName}`;
	const hashSeparator = "################################################################";
...
const portHttp = new Set([80, 8080, 8880, 2052, 2086, 2095, 2082]);
const portHttps = new Set([443, 8443, 2053, 2096, 2087, 2083]);

function getvlessSub(User_ID_Path, hostname์) {
	const User_ID_Array = User_ID_Path.includes(',') ? User_ID_Path.split(',') : [User_ID_Path];
	const General_URL_Http = `?encryption=none&security=none&fp=random&type=ws&host=${hostname์}&path=/vless-ws#`;
	const General_URL_Https = `?encryption=none&security=tls&sni=${hostname์}&fp=random&type=ws&host=${hostname์}&path=/vless-ws#`;

	const Result์ = User_ID_Array.flatMap((User_ID้) => {
		const ConfigurationHttp = Array.from(portHttp).flatMap((port) => {
			if (!hostname์.includes('pages.dev')) {
				const Url_part = `${hostname์}-HTTP-${port}`;
				const vlessmainHttp = atob(pt) + '://' + User_ID้ + atob(at) + hostname์ + ':' + port + General_URL_Http + Url_part;
				return proxyIPS.flatMap((proxyIP) => {
					const vlessSecondaryHttp = atob(pt) + '://' + User_ID้ + atob(at) + proxyIP + ':' + port + General_URL_Http + Url_part + '-' + proxyIP + '-' + atob(ed);
		...
		const ConfigurationHttps = Array.from(portHttps).flatMap((port) => {
			const Url_part = `${hostname์}-HTTPS-${port}`;
			const vlessmainHttps = atob(pt) + '://' + User_ID้ + atob(at) + hostname์ + ':' + port + General_URL_Https + Url_part;
			return proxyIPS.flatMap((proxyIP) => {
				const vlessSecondaryHttps = atob(pt) + '://' + User_ID้ + atob(at) + proxyIP + ':' + port + General_URL_Https + Url_part + '-' + proxyIP + '-' + atob(ed);
